...
class WebpToMP4Converter:
    ...
    def on_drop(self, event):
        ...
        # Get the dropped files (handles both Windows and other platforms)
        if event.data:
            # Parse the dropped files - this handles the special Tkinter DND format
            if isinstance(event.data, str):
                # On Windows, files are separated by spaces and wrapped in {}
                files = self.parse_dropped_files(event.data)
            else:
                # On other platforms, it might be a list
                files = event.data if isinstance(event.data, list) else [event.data]
            
            # Add each valid WebP file
            for file_path in files:
                if file_path.endswith('.webp') and os.path.isfile(file_path):
    ...
    def on_drag_enter(self, event):
        ...
        self.listbox.configure(background='lightblue')
        return 'copy'
    ...
    def parse_dropped_files(self, data):
        """Parse the special Tkinter DND format on Windows"""
        files = []
...
if __name__ == "__main__":
    if HAS_DND:
        root = TkinterDnD.Tk()  # Uses the drag-and-drop enabled window
    else:
        root = tk.Tk()
